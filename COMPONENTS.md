...
Human-oriented usage guide for `@tollerud/ui` **v5.3.0**. Components ship as React `.tsx` modules with matching CSS in `globals.css` / `tokens.css`.
...
| `TopNav` | Branded monogram lockup, nav links, and top-level actions; `TopNavAction` controls mobile placement. `TopNavItem.icon` (≥ 5.1.0) shows in dropdown/mobile rows; `TopNavItem.onClick` (≥ 5.2.0) renders the row as a button instead of a link, e.g. sign out. `mobileMenuSections` (≥ 5.1.0) adds labeled row groups to the mobile sheet; `userMenu` (≥ 5.3.0) renders a single account/user menu from one data structure — a `DropdownMenu` next to the desktop actions, and the same sections appended to the mobile sheet automatically; `mobileMenuExtra` injects fully custom content at the bottom. A `navItems` entry with `items: TopNavItem[]` becomes a flyout group (desktop `NavigationMenu` with a trigger-tracking indicator caret, mobile accordion) — requires the `@radix-ui/react-navigation-menu` peer (≥ 4.19.0) |
