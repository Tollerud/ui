...
- **Portalled dropdowns work inside Radix Dialog (≥ 4.8.36)** — `Combobox`, `Select`, and `DatePicker` dropdowns no longer close the dialog on click, and search inputs (e.g. `Combobox` `searchPlacement="dropdown"`) can now receive focus normally. Fixed via native `pointerdown`/`focusin` stop-propagation on the portal element; no consumer changes needed.
