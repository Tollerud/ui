...
Dark, monochrome + single yellow-accent UI library ("noir" aesthetic). **v4.8.38** — Combobox dropdown search auto-focus fixed inside Dialog (callback ref + useLayoutEffect escape hatch). This skill documents the package's **actual current exports** (verified against `components/index.ts` in the source repo) — not aspirational docs. If you see a component referenced elsewhere that isn't listed below, it does not exist yet; don't import it.
...
- **Portalled dropdowns work inside Radix Dialog (≥ 4.8.36–4.8.38)** — `Combobox`, `Select`, and `DatePicker` dropdowns no longer close the dialog on click, and `Combobox` `searchPlacement="dropdown"` auto-focuses the search input even when used inside a Dialog. Three-part fix: `FloatingDropdownPortal` stops `pointerdown`/`focusin` from reaching Radix's document handlers (4.8.36); `Combobox` focus timing fixed from `useEffect` to callback ref + `queueMicrotask`, and escape hatch upgraded from `useEffect` to `useLayoutEffect` so it is always in place before focus fires (4.8.38). No consumer API changes.
