...
- **Mobile dropdown float/flip fix (≥ 4.8.54)** — follow-up to 4.8.53. On touch, portalled dropdowns (`Combobox`, `Select`, `DatePicker`, `Segmented`) now place once and stay put instead of repositioning on every scroll tick (iOS momentum/rubber-band/address-bar/focus-zoom made the panel float around). Placement is also sticky: filtering a `Combobox` down to one result no longer flips the panel from top to bottom. Desktop still tracks the anchor on scroll/resize.
