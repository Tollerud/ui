...
**ScrollRail** — horizontal scroll rail for card rows, image strips, and wide content (≥ 4.9.0). `peek?: false | 'sm' | 'md' | 'lg'` (default `'md'`), `gap?: 'xs' | 'sm' | 'md' | 'lg'`, `controls?: boolean | 'auto'`, `fadeEdges?: boolean` (default `true`), `itemWidth?: number | string`, `ariaLabel?: string`, `trackClassName?: string`. Continuous scroll — not snap-carousel. Does not block vertical page scroll on touch.
