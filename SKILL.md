...
- **`Card` compound parts (≥ 4.9.4)** — `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `CardFooter` with darker header/footer bands. Plain `<Card>` padding unchanged. **≥ 4.9.5:** `CardContent` explicitly keeps the raised surface background.
