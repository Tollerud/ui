...
- **Portalled dropdowns work inside Radix Dialog (≥ 4.8.36–4.8.37)** — `Combobox`, `Select`, and `DatePicker` dropdowns no longer close the dialog on click, and `Combobox` `searchPlacement="dropdown"` search input can now receive focus. Two-part fix: `FloatingDropdownPortal` stops `pointerdown`/`focusin` from reaching Radix's document handlers (4.8.36); `Combobox` uses `useEffect` instead of `setTimeout` to focus the search input after React commits the render (4.8.37). — `Combobox`, `Select`, and `DatePicker` dropdowns no longer close the dialog on click, and search inputs (e.g. `Combobox` `searchPlacement="dropdown"`) can now receive focus normally. Fixed via native `pointerdown`/`focusin` stop-propagation on the portal element; no consumer changes needed.
